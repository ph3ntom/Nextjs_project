...
export default function QuestionPage({ params }: QuestionPageProps) {
  ...
  useEffect(() => {
    const fetchQuestionAndAnswers = async () => {
      try {
        // Question 데이터 가져오기
        const questionResponse = await fetch(`http://localhost:3001/questions/${id}`)
        ...
        const answersResponse = await fetch(`http://localhost:3001/questions/${id}/answers`)
      ...
      <div className="mt-10">
        ...
        <div className="space-y-6">
          {answers.map((answer) => (
            ...
            ㅇ      <div className="text-xl font-bold">{answer.votes}</div>
                  <Button variant="ghost" size="icon" className="rounded-full">
                    <ArrowDown className="h-6 w-6" />
                    <span className="sr-only">Downvote</span>
                  </Button>
